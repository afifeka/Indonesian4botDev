...
module.exports = function (client, options) {
	...
	function play(msg, suffix) {
		...
		if (msg.member.voiceChannel === undefined) return message.channel.send({embed: {
			color: 0x474747,
			description: "⛔ | **Mohon masuk ke salah satu voice Channel terlebih dahulu!**"
		}});
		...
		if (queue.length >= musicbot.maxQueueSize) {
			return message.channel.send({embed: {
				color: 0x474747,
				description: "⛔ | **Queue telah melampaui batas.**"
			}});
		}
		...
		msg.channel.send(note('note', 'Searching...')).then(response => {
				...
				response.edit(note('note', 'Playlist detected! Fetching...')).then(response => {
					...
					ypi.playlistInfo(musicbot.youtubeKey, playid, function(playlistItems) {
						...
						function endrun() {
							if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
								const embed = new Discord.MessageEmbed();
								embed.setAuthor(`Song Queue`, message.author.displayAvatarURL());
								...
								if (qvids.length > 1000) qvids = 'Teks karakter lebih dari 1000, replaced...';
								if (svids.length > 1000) svids = 'Teks karakter lebih dari 1000, replaced...';
		    ...
		    search(searchstring, opts, function(err, results) {
					...
					function playStart(videos) {
						...
						response.delete();
						msg.channel.send(`\`\`\`\nMohon masukkan nomor yang tersedia (tanpa prefix), dan ketik **cancel** untuk membatalkannya.\n${text}\n\`\`\``).then(imsg => {
							...
							.then(collected => {
								...
								if (mcon.includes(`cancel`)) return imsg.edit("⛔ | Pencarian musik dibatalkan.")
							...
							.catch(collected => {
								imsg.edit(`\`\`\`\n⛔ | Pencarian dibatalkan, karena ketidakaktifan.\n\`\`\``);
								return;
							});
						});
					};
	...
	function skip(msg, suffix) {
		...
		if (voiceConnection === null) return message.channel.send({embed: {
			color: 0x474747,
			description: "⛔ | **Tidak ada musik yang dimainkan saat ini.**"
		}});

		// Get the queue.
		const queue = getQueue(msg.guild.id);

		if (!canSkip(msg.member, queue)) return message.channel.send({embed: {
			color: 0x474747,
			description: "⛔ | **Anda tidak bisa skip lagu ini karena lagu ini bukan Anda yang memutarkannya.**"
		}}).then((response) => {
		...
		if (first) {
			message.channel.send({embed: {
				color: 0x474747,
				description: '⛔ | Skipped **' + toSkip + '**! (Disabled Looping)'
			}})
		} else {
			message.channel.send({embed: {
				color: 0x474747,
				description: '⛔ | Skipped **' + toSkip + '**!'
			}})
		}
	}
	...
	function queue(msg, suffix) {
		...
		if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
			...
			if (songNum > 0) {
				if (songNum > queue.length) return message.channel.send({embed: {
					color: 0x474747,
					description: '⛔ | Nomor yang anda masukkan salah.'
				}})
				...
				embed.setColor(0x27e33d);
				if (musicbot.requesterName && reqMem) embed.setFooter(`Queued oleh: ${reqMem.username}`, queue[songNum].requesterAvatarURL);
				if (musicbot.requesterName && !reqMem) embed.setFooter(`Queued oleh: \`UnknownUser (id: queue[songNum].requester)\``, queue[songNum].requesterAvatarURL)
				msg.channel.send({embed});
			} else {
				const embed = new Discord.MessageEmbed();
	...
	function pause(msg, suffix) {
		...
		if (!isAdmin(msg.member))
			return message.channel.send({embed: {
				color: 3447003,
				description: "⛔ | Hanya **Administrator** yang dapat menghentikan lagu sementara."
			  }});
	...
	function leave(msg, suffix) {
		...
		if (isAdmin(msg.member)) {
			...
			message.channel.send({embed: {
				color: 3447003,
				description: "⛔ | Berhasil **disconnect!**"
			  }});
		} else {
			message.channel.send({embed: {
				color: 3447003,
	...
	function resume(msg, suffix) {
		...
		message.channel.send({embed: {
			color: 3447003,
			description: "⛔ | Musik berhasil dilanjutkan."
		  }});
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function volume(msg, suffix) {
		...
		if (suffix > 200 || suffix < 0) return message.channel.send({embed: {
			color: 3447003,
			description: "⛔ | Volume tidak dapat lebih dari 100."
		  }}).then((response) => {
			response.delete(5000);
		});
		...
		message.channel.send({embed: {
			color: 3447003,
			description: "⛔ | Volume diganti menjadi" + suffix
		  }});
	...
	function loop(msg, suffix) {
		...
		if (musicbot.loop === "true") {
			...
			message.channel.send({embed: {
				color: 3447003,
				description: ":arrow_forward: | Repeat diaktifkan!"
			  }});
		} else if (musicbot.loop === "false") {
			musicbot.loop = "true";
			message.channel.send({embed: {
				color: 3447003,
				description: ":repeat_one: | Repeat diaktifkan!"
			  }});
		}
	};
